...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try { jsonResp = await resp.json(); } catch { jsonResp = null; }

    if (!resp.ok) {
      console.warn('OpenRouter error:', resp.status, jsonResp);
    ...
    if (Array.isArray(rawContent)) {
      ...
      if (t) modelRawText = t.text || t.content || (typeof t === 'string' ? t : null);
    ...
    if (modelRawText) {
      ...
      try { ocrResult = JSON.parse(cleaned); } catch {
        const first = modelRawText.indexOf('{');
        const last = modelRawText.lastIndexOf('}');
        if (first !== -1 && last !== -1 && last > first) {
          try { ocrResult = JSON.parse(modelRawText.slice(first, last + 1)); } catch {}
        }
      }
      if (!ocrResult) ocrResult = { text: modelRawText };
    }
    ...
    res.status(200).json({ ok: true, idVerificationId: result.insertedId, parsed: normalized, status: insert.status, rawModelText: modelRawText });
  } catch (err) {
